...
export default function Enrollment({match}) {
    ...
    const imageUrl = enrollment.course._id ? `/api/courses/photo/${enrollment.course._id}?${new Date().getTime()}` : '/api/courses/defaultphoto'
    return (
        <div className={classes.root}>
            ...
            ><div classsName={classes.toolbar}/>
                ...
                <List>
                    <ListItem>
                        <ListItemText primary={<div className={classes.progres} ><span>{totalComplete}</span> out of <span>{enrollment.lessonStatus.length}</span>completed</div>} />
                    </ListItem>
                </List>
            </Drawer>
            {values.drawer == -1 && <Card className={classes.card}>
                <CardHeader title={enrollment.course.name} subheader={
                    <div>
                ...
                action={
                    totalComplete == enrollment.lessonStatus.length && (<span className={classes.action}>
                        <Button variant="contained" color="secondary">
                            <CheckCircle/> &nsbp; Completed
                        </Button>
                    </span>)
                } 
                />
                <div className={classes.flex}>
                    ...
                    <div className={classes.details}>
                        <Typography varian="body1" className={classes.subheading} >
                ...
                <div>
                    <CardHeader
                        title={<Typography variant="h6" className={classes.subjeading} >Lessons</Typography>}  
                        subheader={<Typography varriant="body1" className={classes.subheading} >{enrollment.course.lessons && enrollment.course.lessons.length} lessons</Typography>} 
                        action={
                            auth.isAuthenticated().user && auth.isAuthenticated().user._id == enrollment.course.instructor._id && (<span className={classes.action}></span>)
                        } 
                    />
                    <List>
